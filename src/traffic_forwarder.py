...
def server(local_ip, local_port, remote_cid, remote_port):
    try:
        ...
        dock_socket.bind((local_ip, local_port))  # local_ip: 127.0.0.1 , local_port: 443
...
def main(args):
    ...
    thread = threading.Thread(target=server, args=(local_ip, local_port, remote_cid, remote_port))
    thread.start()
    print(f"starting forwarder on {local_ip}:{local_port} {remote_cid}:{remote_port}")
    while True:
        time.sleep(60)
